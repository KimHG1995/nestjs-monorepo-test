...
- `libs/sqs-client/src/sqs-client.service.ts`의 사용되지 않는 제네릭을 제거하고 메시지 본문을 `object`로 제한한다. `Record<string, unknown>`은 인덱스 시그니처가 없는 클래스 DTO를 거부하므로 사용하지 않는다.
- `libs/common-utils`에 중첩된 JSON 값만 허용하는 재귀 `JsonValue` 타입과 `JsonValueSchema`, 객체 전용 `JsonObjectSchema`를 추가한다.
- `apps/api-server/src/track-activity.dto.ts`와 `apps/activity-worker/src/activity-worker.service.ts`가 동일한 `JsonObjectSchema`를 사용하도록 변경한다.
- 공용 JSON 스키마는 문자열, 숫자, boolean, null, 배열 및 객체의 재귀 조합을 허용하고 undefined, bigint, 함수 및 Date 같은 JSON 비호환 값을 거부하는 단위 테스트를 갖는다.

HTTP 본문과 SQS `JSON.parse` 결과는 원래 JSON 호환 값만 전달할 수 있으므로 런타임 계약은 바뀌지 않는다. 재귀 타입을 사용하면 Worker의 검증 결과가 Prisma JSON 입력 타입과도 안전하게 호환된다.
...
- `const` 변수는 `camelCase` 또는 `UPPER_CASE`를 사용한다. 기존 공개 Zod 스키마, 생성자 및 NestJS 데코레이터 팩터리처럼 타입 또는 생성 역할을 하는 값은 `PascalCase`를 허용한다.
...
- `libs/common-utils/src/schemas/json-value.schema.ts`
- `libs/common-utils/src/schemas/json-value.schema.spec.ts`
