...
기존 `UserActivity.productId`는 nullable이므로 기존 행은 그대로 유지된다. 새 마이그레이션은
`products` 테이블, nullable 외래 키, 복합 인덱스를 추가한다. 생성된 Prisma Client는 기존
정책대로 Git에 포함하지 않고, 마이그레이션 SQL과 ERD만 추적한다.
